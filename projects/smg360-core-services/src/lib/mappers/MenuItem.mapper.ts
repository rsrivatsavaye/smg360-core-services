...
export class MenuItemMapper {
    ...
    static map(items: Array<MenuItem>, viewService: ViewService, localStorageService: LocalStorageService, translationService: TranslateService, cookieService: CookieService): Array<MenuItem> {
      const settings = viewService.getCurrentViewFeatures();
      let returnItems = [];
      ...
      */
      const links = returnItems.filter(m => m.menuTextTranslationKey === 'MENU_NAV_V5_LINK');
      if (links && links.length === 1) {
        ...
        const authorization: any = cookieService.getAuthToken(); // ?? localStorageService.getObjectItem('ls.authorizationData');
        let userName = '';
        const accessToken = authorization.token;
        const token = JSON.parse(window.atob(accessToken.split('.')[1]));
        userName = token.name;
    ...
    static createAnalyticsObject(label) {
      return {
        ...
        metadata: { category: analyticsMetadataObjects.categories.filterPanel, label }
      };
